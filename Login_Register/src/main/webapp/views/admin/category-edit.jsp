...
<form action="<c:url value='/admin/category/update'></c:url>" method="post" enctype="multipart/form-data">
    ...
    <c:if test="${cate.categoryimages.substring(0,5) != 'https' }">
        <c:url value="/image?fname=${cate.categoryimages}" var="imgUrl"></c:url>
    </c:if> 
    <c:if test="${cate.categoryimages.substring(0,5) == 'https' }">
        <c:url value="${cate.categoryimages}" var="imgUrl"></c:url>
    </c:if> 
    <img id="images" height="150" width="200" src="${imgUrl}" /><br>
    <input type="file" onchange="chooseFile(this)" id="image" name="image" value="${cate.categoryimages}"><br>
    <p>Status:</p>
